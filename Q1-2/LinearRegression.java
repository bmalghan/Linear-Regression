...
public class LinearRegression {
  ...
  public double currentCost(){
    ...
    for (int i = 0; i < nbreOfSamples; i++){
      //summation
      epsilon += Math.pow(hypothesis(samples[i])-(samplesValues[i]),2);
    }
    //cost divided by total number of samples
    double cost = epsilon/nbreOfSamples;
    return cost;
  }
  ...
  public void gradientDescent(double alpha, int numberOfSteps) {
    ...
    for (int i = 0; i < numberOfSteps; i++) {
      ...
      for(int j = 0; j < nbreOfSamples; j++) {
        //theta 0 -- Summation
        sumA += hypothesis(samples[j])-samplesValues[j];
        //theta 1 -- Summation
        sumB += samples[j]*(hypothesis(samples[j])-samplesValues[j]);
        
      }
      //theta 0 final
      theta0 = theta0 -alpha*(2*sumA/nbreOfSamples);
      //theta 1 final
      theta1 = theta1-alpha*(2*sumB/nbreOfSamples);
